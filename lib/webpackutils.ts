...
export class WebpackUtils {
	 ...
	 */
	static getModuleWithKey(filter: Filter): [any, string] {
		let target: any;
		let id: string;
		let key: string;
		Webpack.getModule(
	 ...
	 */
	static expectModule<T>(options: ExpectOptionsWithFilter<T>): T;
	/**
	 * Finds a module using a filter function, and handles the error if the module is not found.
	 * @param filter A function to use to filter modules.
	 ...
	 * @returns The found module or the fallback if the module cannot be found.
	 */
	static expectModule<T>(filter: Filter, options?: ExpectOptions<T>): T;
	static expectModule<T>(filterOrOptions: Filter | ExpectOptionsWithFilter<T>, options?: ExpectOptions<T>): T {
		let filter: Filter;
	 ...
	 * @returns The generated filter.
	 */
	static byValues(...filters: Filter[]) {
